...
в настройках запуска программы в вашей IDE. В случае IntellijIDEA, например, эта опция указывается
...
(и, вероятно, вспомогательные классы и интерфейсы) и отправить их код в форму. Все классы должны быть публичными и
...
//   где по получателю можно получить список зарплат, которые были ему отправлены.
    ...
    public static class MailMessage implements Info<String> {
    ...
    public static class Salary implements Info <Integer> {
        private final String from;
        private final String to;
    ...
    public static class MailService<T> implements Consumer<Info<T>> {
